...
namespace OmniSharp.TestCommon
{
    public static class WorkspaceHelpers
    {
        ...
        {
            return CreateSimpleWorkspace(PluginHostHelpers.CreatePluginHost(), sourceFiles);
        }

        public static OmnisharpWorkspace CreateSimpleWorkspace(CompositionHost host, Dictionary<string, string> sourceFiles)
        {
            host = host ?? PluginHostHelpers.CreatePluginHost(typeof(CodeCheckService).GetTypeInfo().Assembly);
